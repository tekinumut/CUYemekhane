 ...
 */
class DailyMenuUseCase @Inject constructor(
    @IODispatcher private val dispatcher: CoroutineDispatcher,
    private val mainPageRepository: MainPageRepository,
    private val mainPageMapper: MainPageMapper
) : FlowUseCase<Unit, Resource<DailyMenuUIModel>>(dispatcher) {

    override suspend fun getExecutable(params: Unit): Flow<Resource<DailyMenuUIModel>> {
        return flow {
    ...
    private fun getMappedResponse(resource: Resource<MainPageResponse>): Resource<DailyMenuUIModel> {
        return when (resource) {
            ...
            is Resource.Success -> {
                ...
                Resource.Success(mainPageUIModel.dailyMenu)
            }
        }
    }
}